...
int main(void) {
	...
	if(i==0){
		sprintf(operation, "new_state REC_ON %s", PASSWORD);
	}
        // Alterna tra add_path e remove_path
        else if (i % 2 == 0) {
            sprintf(operation, "add_path %s %s", PATH, PASSWORD);
        } else {
            sprintf(operation, "remove_path %s %s", PATH, PASSWORD);
        }
